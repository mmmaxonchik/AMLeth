...
class Account(EtherScanApi):
    ...
    async def get_normal_transactions(self):
        ...
        response = await self._send_request(url)
        return list(map(NormalTransaction.from_dict, self.handle_response(response)["result"]))
    ...
    async def get_internal_transactions(self):
        ...
        response = await self._send_request(url)
        return list(map(InternalTransaction.from_dict, self.handle_response(response)["result"]))
    ...
    async def get_erc20_transfer_events(self):
        ...
        response = await self._send_request(url)
        return list(map(ERC20TransferEvent.from_dict, self.handle_response(response)["result"]))
